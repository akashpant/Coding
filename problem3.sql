...
(default,4,5,date('2019/12/11'),1900);
  ...
  create table copy2				/* here i have created a copy2 table*/
  (
   customer_name varchar(200) not null,
   amount int not null
   );
	insert into copy2 select first_name customer_name,amount from customer inner join seller inner join sales on customer.id=seller.id where customer_id=seller_id;	            /*inserting all the elements from   
    						                previous result to the table copy2 */

create table copy3			/* here i have created a copy2 table*/
  (
   customer_name varchar(200) not null,
   sales_amount int not null
   );
insert into copy3  select customer_name,sum(amount) from copy2 group by customer_name;
/*previous result to the table copy2 */
select* from copy3 order by amount as sales_amount asc;
